...
FixGCOTI::FixGCOTI(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
void FixGCOTI::init()
{
  ...
  gfac1 = gamma/2.0*dt - 1.0;
  dtf = dt*dt;
  gfac2 = 1.0 + gamma/2.0*dt;
  kgfac = 2.0/3.0*kspr*PI*PI*dtf/Nmon/Nmon;
...
void FixGCOTI::initial_integrate(int vflag)
{
  ...
  for(i = me*ncperproc; i < me*ncperproc+ncproc; i++) {
   if(update->ntimestep > 1) {
     // j=0 mode
     j = 0;
     xfn[0] = (2.0*xf_proc[i*ncoll+j][0] + gfac1*xfpproc[i*ncoll+j][0] + 
	    ff_proc[i*ncoll+j][0]*dtf)/gfac2;
     xfn[1] = (2.0*xf_proc[i*ncoll+j][1] + gfac1*xfpproc[i*ncoll+j][1] +
	    ff_proc[i*ncoll+j][1]*dtf)/gfac2;
     xfn[2] = (2.0*xf_proc[i*ncoll+j][2] + gfac1*xfpproc[i*ncoll+j][2] +
	    ff_proc[i*ncoll+j][2]*dtf)/gfac2;     
     ...
     // remaining modes
     for(j = 1; j < ncoll; j++) {
       xfn[0] = (xf_proc[i*ncoll+j][0]*(2.0 - kgfac*j*j) + 
                 xfpproc[i*ncoll+j][0]*(gfac1 - kgfac*j*j) +
		 ff_proc[i*ncoll+j][0]*dtf)/(gfac2 + kgfac*j*j);
       xfn[1] = (xf_proc[i*ncoll+j][1]*(2.0 - kgfac*j*j) + 
                 xfpproc[i*ncoll+j][1]*(gfac1 - kgfac*j*j) +
		 ff_proc[i*ncoll+j][1]*dtf)/(gfac2 + kgfac*j*j);
       xfn[2] = (xf_proc[i*ncoll+j][2]*(2.0 - kgfac*j*j) + 
                 xfpproc[i*ncoll+j][2]*(gfac1 - kgfac*j*j) +
		 ff_proc[i*ncoll+j][2]*dtf)/(gfac2 + kgfac*j*j);     
       xfpproc[i*ncoll+j][0] = xf_proc[i*ncoll+j][0];
       xfpproc[i*ncoll+j][1] = xf_proc[i*ncoll+j][1];
       xfpproc[i*ncoll+j][2] = xf_proc[i*ncoll+j][2];
       xf_proc[i*ncoll+j][0] = xfn[0];
     ...
     for(j = 1; j < ncoll; j++) {
       ...
       xfpproc[i*ncoll+j][2] = xf_proc[i*ncoll+j][2];
       xf_proc[i*ncoll+j][0] = ff_proc[i*ncoll+j][0]*dtf +
                                 xfpproc[i*ncoll+j][0]*(1.0-3.0*kgfac*j*j);
       xf_proc[i*ncoll+j][1] = ff_proc[i*ncoll+j][1]*dtf +
                                 xfpproc[i*ncoll+j][1]*(1.0-3.0*kgfac*j*j);
       xf_proc[i*ncoll+j][2] = ff_proc[i*ncoll+j][2]*dtf +
                                 xfpproc[i*ncoll+j][2]*(1.0-3.0*kgfac*j*j);
     }
   }
  }
...
void FixGCOTI::create_arrays(int natoms)
{
  xfpproc =
    memory->create_2d_double_array(natoms,3,"fix_gcoti:xfpproc");
  xr_loc =
    memory->create_2d_double_array(natoms,3,"fix_gcoti:xr_loc");
  xr_all =
    memory->create_2d_double_array(natoms,3,"fix_gcoti:xr_all");
  xf_proc =
    memory->create_2d_double_array(natoms,3,"fix_gcoti:xf_proc");
  fr_loc =
    memory->create_2d_double_array(natoms,3,"fix_gcoti:fr_loc");
  fr_all =
    memory->create_2d_double_array(natoms,3,"fix_gcoti:fr_all");
  ff_proc =
    memory->create_2d_double_array(natoms,3,"fix_gcoti:ff_proc");
} 
