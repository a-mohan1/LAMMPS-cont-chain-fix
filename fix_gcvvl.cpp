...
drag uses intermediate velocity (lammps implementation) */
...
FixGCVVL::FixGCVVL(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 7) error->all("Illegal fix command");
...
void FixGCVVL::initial_integrate(int vflag)
{
  ...
  // ff_proc = forces in fourier space for chains on this proc
  // pair+thermal before 1st time step
  // pair+thermal+drag after call to final_integrate
  ...
  for(i = me*ncperproc; i < me*ncperproc+ncproc; i++) {
    if(update->ntimestep > 1) { 
     ...
     for(j = 1; j < ncoll; j++) {
       vf_proc[i*ncoll+j][0] = vf_proc[i*ncoll+j][0] + 
	 dtf*(-kfac*j*j*xf_proc[i*ncoll+j][0] + ff_proc[i*ncoll+j][0]);
       vf_proc[i*ncoll+j][1] = vf_proc[i*ncoll+j][1] + 
	 dtf*(-kfac*j*j*xf_proc[i*ncoll+j][1] + ff_proc[i*ncoll+j][1]);
       vf_proc[i*ncoll+j][2] = vf_proc[i*ncoll+j][2] + 
	 dtf*(-kfac*j*j*xf_proc[i*ncoll+j][2] + ff_proc[i*ncoll+j][2]);
       xf_proc[i*ncoll+j][0] += dt*vf_proc[i*ncoll+j][0];
       xf_proc[i*ncoll+j][1] += dt*vf_proc[i*ncoll+j][1];
       xf_proc[i*ncoll+j][2] += dt*vf_proc[i*ncoll+j][2];
     }
    } else { // 1st time step, zero initial velocities
     // j=0 mode
     j = 0;
...
void FixGCVVL::final_integrate()
{
  ...
  for(i = me*ncperproc; i < me*ncperproc+ncproc; i++) {
    ...
    // add drag force for chain i
    for(j = 0; j < ncoll; j++) {
      ff_proc[i*ncoll+j][0] += -gamma*vf_proc[i*ncoll+j][0];
      ff_proc[i*ncoll+j][1] += -gamma*vf_proc[i*ncoll+j][1];
      ff_proc[i*ncoll+j][2] += -gamma*vf_proc[i*ncoll+j][2];
    }
  }
...
void FixGCVVL::create_arrays(int natoms)
{
  xr_loc =
    memory->create_2d_double_array(natoms,3,"fix_gcvvl:xr_loc");
  xr_all =
    memory->create_2d_double_array(natoms,3,"fix_gcvvl:xr_all");
  xf_proc =
    memory->create_2d_double_array(natoms,3,"fix_gcvvl:xf_proc");
  fr_loc =
    memory->create_2d_double_array(natoms,3,"fix_gcvvl:fr_loc");
  fr_all =
    memory->create_2d_double_array(natoms,3,"fix_gcvvl:fr_all");
  ff_proc =
    memory->create_2d_double_array(natoms,3,"fix_gcvvl:ff_proc");
  vf_proc =
    memory->create_2d_double_array(natoms,3,"fix_gcvvl:vf_proc");
} 
