...
langevin thermostat
1st order explicit time-stepping
*/
...
FixGCEXP::FixGCEXP(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
void FixGCEXP::init()
{
  ...
  kgfac = 2.0*kspr*PI*PI*dtf/Nmon/Nmon;
...
void FixGCEXP::initial_integrate(int vflag)
{
  ...
  for(i = me*ncperproc; i < me*ncperproc+ncproc; i++) {
   if(update->ntimestep > 1) {
     ...
     for(j = 1; j < ncoll; j++) {
       ...
       xfpproc[i*ncoll+j][2] = xf_proc[i*ncoll+j][2];
       xf_proc[i*ncoll+j][0] = ff_proc[i*ncoll+j][0]*dtf +
                                 xfpproc[i*ncoll+j][0]*(1.0-kgfac*j*j);
       xf_proc[i*ncoll+j][1] = ff_proc[i*ncoll+j][1]*dtf +
                                 xfpproc[i*ncoll+j][1]*(1.0-kgfac*j*j);
       xf_proc[i*ncoll+j][2] = ff_proc[i*ncoll+j][2]*dtf +
                                 xfpproc[i*ncoll+j][2]*(1.0-kgfac*j*j);
     }
   }
  }
...
void FixGCEXP::create_arrays(int natoms)
{
  xfpproc =
    memory->create_2d_double_array(natoms,3,"fix_gcexp:xfpproc");
  xr_loc =
    memory->create_2d_double_array(natoms,3,"fix_gcexp:xr_loc");
  xr_all =
    memory->create_2d_double_array(natoms,3,"fix_gcexp:xr_all");
  xf_proc =
    memory->create_2d_double_array(natoms,3,"fix_gcexp:xf_proc");
  fr_loc =
    memory->create_2d_double_array(natoms,3,"fix_gcexp:fr_loc");
  fr_all =
    memory->create_2d_double_array(natoms,3,"fix_gcexp:fr_all");
  ff_proc =
    memory->create_2d_double_array(natoms,3,"fix_gcexp:ff_proc");
} 
